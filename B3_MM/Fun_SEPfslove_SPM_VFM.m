...
function f= Fun_SEPfslove_SPM_VFM(x,preset,state,basevalue)
    ...
    PCOI=preset.isnVFM'*(Pm-Pe);   
    

    f(1)=Pm(1)-Pe(1)-m(1)/sum(m)*PCOI+m(1)/sum(m)*sum(d)*domegacoi-d(1)*domegacoi;
    f(2)=Pm(2)-Pe(2);

    f(ngen)= PCOI-sum(d)*domegacoi;    % the nth equ
