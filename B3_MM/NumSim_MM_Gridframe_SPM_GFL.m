...
M = diag([ones(2,1); ones(14,1)*1e-15; ones(4,1)]);
options = odeset('Mass',M,'RelTol',1e-10,'AbsTol',[1e-8*ones(1,2),1e-12*ones(1,14),1e-8*ones(1,4)]);
% prefault
    system="prefault";
    delta0=prefault.SEP_delta;
    omega0=prefault.SEP_omegapu*omegab*ones(ngen,1);
    ...
    pll0 = prefault.pll;
    xint0 = prefault.xint;
    x0=[delta0; delta_net0; voltage_net0; omega0; pll0; xint0];
    [t_prefault, x_prefault_all] = ode15s(@(t,x)f_timedomain_DAE(t, x, system),0:Iter.Tunit:Iter.Tfault,x0,options);
%% fault
    system="fault1";
    delta0=x_prefault_all(end,1:2)';
    omega0=x_prefault_all(end,17:18)';
    delta_net0=x_prefault_all(end,3:9)';
    voltage_net0=x_prefault_all(end,10:16)';
    pll0=x_prefault_all(end,19)';
    xint0=x_prefault_all(end,20)';
    delta_net0(fault.faultbus-ngen)=[];
    voltage_net0(fault.faultbus-ngen)=[];
    delta_net0(1)=pll0+pi/8;
    temp_ini= [delta_net0;voltage_net0];
    delta_net0(7)=0;
    voltage_net0(7)=0;
    [delta_net_s,V_net_s,flag_iter,n_iter,err] = Fun_AEiteration_SPM_GFL(delta_net0,voltage_net0,delta0,pll0,preset,Basevalue,system,1e5,1e-10);
    [Results_fsolve,fval,exitflag,output]=fsolve(@(x)Fun_AEfslove_SPM_GFL(x,delta0,pll0,preset,system),temp_ini,optimset('TolFun',1e-10,'MaxFunEvals',1e5,'Maxiter',1e5,'TolX',1e-5));
    [t_fault, x_fault_all] = ode15s(@(t, x) f_timedomain_DAE(t, x, system),Iter.Tfault:Iter.Tunit:Iter.Trecover,[delta0; delta_net_s; V_net_s; omega0; pll0; xint0],options);
    fault_delta_net=x_fault_all(2:end,3:8);
    ...
    fault_voltage_net=x_fault_all(2:end,10:15);
    ...
    fault_delta_gen = x_fault_all(2:end,1:2);
    fault_pll = x_fault_all(2:end,19);
    fault_xint = x_fault_all(2:end,20);

    fault_delta_net = mod(fault_delta_net + pi, 2*pi) - pi;
%% fault-clear
    system = "postfault";
    nbus = preset.nbus;
    ...
    for i = 1:(size(fault_delta_gen,1))
       ...
       [Results_fsolve,fval,exitflag,output]=fsolve(@(x)Fun_AEfslove_SPM_GFL(x,fault_delta_gen(i,:)',fault_pll(i,:)',preset,system),temp_ini,optimset('TolFun',1e-20,'MaxFunEvals',1e5,'Maxiter',1e5,'Display','off','TolX',1e-5));
       if exitflag<=0
           error('cannot find fault-clear state! \n');
       end
       delta_net_faultclear(i,:) = Results_fsolve(1:7)';
       voltage_net_faultclear(i,:) = Results_fsolve(8:14)';
       temp_ini = Results_fsolve;
    end

%% postfault
    system="postfault";
    delta0=x_fault_all(end,1:2)';
    omega0=x_fault_all(end,17:18)';
    pll0 = x_fault_all(end,19)';
    xint0 = x_fault_all(end,20)';
    [delta_net_s,Voltage_net_s,flag_iter,n_iter,err] = Fun_AEiteration_SPM_GFL(temp_ini(1:7),temp_ini(8:14),delta0,pll0,preset,Basevalue,system,1e4,1e-10);
    ...
    [t_postfault, x_postfault_all] = ode15s(@(t,x)f_timedomain_DAE(t,x,system),Iter.Trecover:Iter.Tunit:Iter.Ttotal,[delta0; delta_net_s; Voltage_net_s; omega0;pll0;xint0],options);
    %clear delta0 omega0

%% data collection
     t_timedomain_DAE = [t_prefault(2:end);t_fault(2:end);t_postfault(2:end)];
     deltac_timedomain_DAE  = [x_prefault_all(2:end,1:2); fault_delta_gen; x_postfault_all(2:end,1:2)];
     omega_timedomain_DAE  = [x_prefault_all(2:end,17:18); x_fault_all(2:end,17:18); x_postfault_all(2:end,17:18)]./omegab;
     omegacoi_timedomain_DAE  = omega_timedomain_DAE *preset.m./sum(preset.m);
     omegac_timedomain_DAE  = omega_timedomain_DAE  - omegacoi_timedomain_DAE *ones(1,2);
     delta_net_DAE = [x_prefault_all(2:end,3:9); fault_delta_net; x_postfault_all(2:end,3:9)];
     voltage_net_DAE = [x_prefault_all(2:end,10:16); fault_voltage_net; x_postfault_all(2:end,10:16)];

     delta_net_clear_DAE = [x_prefault_all(2:end,3:9); delta_net_faultclear; x_postfault_all(2:end,3:9)];
     voltage_net_clear_DAE = [x_prefault_all(2:end,10:16); voltage_net_faultclear; x_postfault_all(2:end,10:16)];

     pllc_timedomain_DAE = [x_prefault_all(2:end,19); fault_pll; x_postfault_all(2:end,19)];
     xint_timedomain_DAE = [x_prefault_all(2:end,20); fault_xint; x_postfault_all(2:end,20)];
     Vq_timedomain_DAE = -1*voltage_net_DAE(:,1).*sin(pllc_timedomain_DAE-delta_net_clear_DAE(:,1));

     omegac_pll = (xint_timedomain_DAE+ preset.Kp_pll*Vq_timedomain_DAE)./omegab -omegacoi_timedomain_DAE + ones(size(omegacoi_timedomain_DAE,1),1);

     pllc_timedomain_DAE = mod(pllc_timedomain_DAE + pi, 2*pi) - pi;
...
% deltac
    ...
    plot(t_timedomain_DAE,pllc_timedomain_DAE(:,1),'linewidth',2,'color',[0/255 175/255 0/255]);    hold on;
    ...
    ax.FontName='Arial';
    ax.FontSize=14;
    legend("\delta_1","\delta_2","\delta_{pll}");
...
%% omega
    ...
    plot(t_timedomain_DAE,omegac_pll,'linewidth',2,'color',[0/255 175/255 0/255]);    hold on;
...
%     plot(TM_post,omegacoi_RK4-1,'LineStyle',':','linewidth',2,'color',[0/255 0/255 0/255]);    hold on;
    ...
    ax.FontName='Arial';
    ax.FontSize=14;
    legend("\omega_1","\omega_2","\omega_{pll}","\omega_{coi}");
    xlabel('Time(s)','FontSize',18,'FontName','Times New Roman','FontAngle','italic','FontWeight','bold','position',[xlab_x,xlab_y,-1]);
    ...
    plot(t_timedomain_DAE,delta_net_DAE(:,7),'linewidth',2,'color',[0 0.4470 0.7410]);    hold on;
    ...
    plot(t_fault(2:end),delta_net_faultclear(:,7),'LineStyle',':','linewidth',2,'color',[0 0.4470 0.7410]);    hold on;
    ...
    for i = 1:7
        legendEntries{i} = ['Bus' num2str(i+2)];
    end
    ...
    plot(t_timedomain_DAE,voltage_net_DAE(:,7),'linewidth',2,'color',[0 0.4470 0.7410]);    hold on;
    ...
    plot(t_fault(2:end),voltage_net_faultclear(:,7),'LineStyle',':','linewidth',2,'color',[0 0.4470 0.7410]);    hold on;
...
%% pll
    ...
    set(gca,'position',[0.115,0.12,0.815,0.84]);
    set(gcf,'position',[60 200 600 450]);
    plot(t_timedomain_DAE,delta_net_DAE(:,1),'linewidth',2,'color',[0/255 95/255 255/255]);    hold on;
    plot(t_timedomain_DAE,pllc_timedomain_DAE,'linewidth',2,'color',[255/255 135/255 0/255]);    hold on;
    plot(t_timedomain_DAE,Vq_timedomain_DAE,'linewidth',2,'color',[0/255 175/255 0/255]);    hold on;
    plot(t_timedomain_DAE,xint_timedomain_DAE/preset.Ki_pll,'linewidth',2,'color',[0 0.4470 0.7410]);    hold on;
...
function dfdt = f_timedomain_DAE(t,x,system)
    dfdt = F_3M9B_SP_DAE_GFL(x,system);
end
