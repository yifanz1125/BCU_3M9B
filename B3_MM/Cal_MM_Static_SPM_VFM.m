...
% 9 bus sys===========================================
    ...
    preset.m= [0.1254;0;0.016];  
    preset.d=preset.m.*[1;1;1];%[0.1;0.1;0.1];
...
%%%%%%% 思考一下怎么把这几项融到powerflow计算中 %%%%%%%%%
    ...
    preset.isnVFM = [1;0;1];%SG2 is VFM
    preset.noVFM = 2;%SG2 is VFM

    %VFM
    preset.kpp = 2.5*2*pi;
    preset.kip = 10;
    Y_dc = 25;
    preset.C_dc = Y_dc/Basevalue.omegab;
...
%         clear flag_iter n_iter
    ...
    elseif(preset.EquCal==2)
        ...
        Results_fsolve=fsolve(@(x)Fun_SEPfslove_SPM_VFM(x,preset,prefault,Basevalue),x_init,options);
        ...
        prefault.SEP_ydc= (prefault.SEP_omegapu-1)*Basevalue.omegab;
        ...
        Results_fsolve=fsolve(@(x)Fun_SEPfslove_SPM_VFM(x,preset,postfault,Basevalue),x_init,options);
        ...
        postfault.SEP_ydc= (postfault.SEP_omegapu-1)*Basevalue.omegab/preset.kip;
        clear delta_tmp Results_fsolve
    end
