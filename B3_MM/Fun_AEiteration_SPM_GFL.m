...
function [delta_net_s,V_net_s,flag_iter,n_iter,err] = Fun_AEiteration_SPM_GFL(delta_net0,V_net0,deltac,pll,preset,basevalue,system,n_itermax,Tolerr)
...
while(flag_iter==0)
    ...
    for i=1:(nbus-ngen)
        ...
        for h=1:preset.n_gfl
            if (preset.no_gfl(h)==Transform(i+ngen))
                J11(i,i)=J11(i,i)- V_net(i)*Id(h)*sin(pll(h)-delta_net(i)) - V_net(i)*Iq(h)*cos(pll(h)-delta_net(i));
                J12(i,i)=J12(i,i)- Id(h)*cos(pll(h)-delta_net(i)) + Iq(h)*sin(pll(h)-delta_net(i));
                J21(i,i)=J21(i,i) + V_net(i)*Iq(h)*sin(pll(h)-delta_net(i)) - V_net(i)*Id(h)*cos(pll(h)-delta_net(i));
                J22(i,i)=J22(i,i) + Iq(h)*cos(pll(h)-delta_net(i)) + Id(h)*sin(pll(h)-delta_net(i));
